...
from vne_simulator.data import VirtualNetwork, PhysicalNetwork
from vne_simulator.base import Controller, Recorder, Counter, Solution
...
class Chromosome(Individual):
    ...
    def calc_fitness(self, solution):
        # minimize
        if solution['result'] and solution['v_net_violation'] <= 0:
            return solution['v_net_cost'] / solution['v_net_revenue']
        # useful infeasible solution
        elif solution['result'] and solution['v_net_violation'] > 0:
            return solution['v_net_cost'] / solution['v_net_revenue'] + solution['v_net_violation'] / solution['v_net_revenue'] * 50
        elif not solution['result']:
            return INFEASIBLE_FITNESS

    def update_best_solution(self, ):
        if self.solution['v_net_violation'] <= 0 and self.fitness < self.best_fitness:
...
class ISGeneticAlgorithmSolver(GeneticAlgorithmSolver):
    ...
    def __init__(self, controller: Controller, recorder: Recorder, counter: Counter, **kwargs):
        super(ISGeneticAlgorithmSolver, self).__init__(controller, recorder, counter, **kwargs)
    ...
    def update_best_individual(self, population):
        ...
        for individual in population:
            if individual.best_solution['v_net_violation'] <= 0 and individual.best_fitness < self.best_individual.best_fitness:
    ...
    def conclusion(self):
        for c in self.chromosomes:
            self.controller.unsafely_deploy_with_node_slots(c.v_net, c.p_net, 
...
def single_meta_run(i, mp_res_dict, solver, v_net, p_net):
    ...
    for iter_id in range(solver.max_iteration):
        ...
        num_infeasible_solutions = sum([1 if c.solution['v_net_violation'] > 0 or not c.solution['result'] else 0 for c in solver.chromosomes])
